...
switch(season) {
    ...
    case 'autumn':
        if(time === 'day'){
            if(group === 'no'){
                selectedCategory = AutNoMorn;
            }else{
                selectedCategory = AutYesMorn;
            }
        }else{
            if(group === 'no'){
                selectedCategory = AutNoNight;
            }else{
                selectedCategory = AutYesNight;
            }
        }
        break;    
}
...
title.innerHTML = `Your perfect date during <span class="word" title="id: season">${season}</span> and  <span class="word" title="id: time">${time}</span>`;

const i = Math.floor(Math.random() * selectedCategory.length + 1);
const message = selectedCategory[i]
const test = document.getElementById('test');
test.innerHTML = message;

//const message = selectedCategory[Math.floor(Math.random() * selectedCategory.length() + 1)];
